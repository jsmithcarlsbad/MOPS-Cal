...
// Coil winder — hub is a SEPARATE FDM part (print partnum 7), not integrated into the coil former.
// Per axis there are 2 coils (Helmholtz pair: left + right along the pair axis). EACH coil gets 2 hub plates — one on
// each flat face of that coil’s inner ring (sandwich). Insert pockets are mirrored on BOTH faces of EACH coil (not only
// one side of the assembly) so left and right coils are each windable with plates on both faces.
// Full pair per axis: 2 coils × 2 plates = 4× partnum 7 prints (same geometry each time unless you add features).
// M3 heat-set inserts: pilot hole 4.2 mm dia (set below), ≥5 mm deep; require coilFormerTotalThickness > 2*winderHeatInsertDepth
// so blind holes from opposite faces do not overlap (or reduce depth per face).
// (1) M3 screws through hub → inserts in former inner ring.
// (2) No printed shaft — 8 mm through-hole + metal flange (e.g. https://www.amazon.com/dp/B0DN6NWQXK — measure PCD/holes).
winderCouplingCenterHoleD		= 8.2;			// mm through hole (clearance for 8 mm OD coupling / shaft)
winderHubDiskThickness			= 4;			// mm hub plate thickness
winderMountScrewD				= 3;			// M3 machine screw (former attachment)
winderMountScrewClearanceD		= 3.4;			// through hole in hub for M3 (former)
winderHeatInsertHoleDiameter	= 4.2;			// mm hole for M3 heat-set insert (per your insert spec)
winderHeatInsertDepth			= 5;			// mm blind depth (≥5 per your spec; match insert length)
winderMountScrewCount			= 3;			// screws to former (bolt circle on inner ring)
winderHoleAngleOffsetDeg		= 15;			// rotate former screw pattern vs spokes (deg)
// Metal flange / coupling (EPLZON-style): clearance holes on a smaller bolt circle — MUST match hardware
winderMetalFlangeBoltCircleRadius	= 16;		// mm — placeholder; measure from your flange with calipers
winderMetalFlangeBoltHoleD		= 3.4;			// clearance for flange screws (e.g. M3); use 4.5 for M4
winderMetalFlangeBoltCount		= 4;			// number of flange mounting holes
winderMetalFlangeAngleOffsetDeg	= 45;			// rotate flange pattern vs former M3 pattern (deg)
useCoilWinderHub				= true;		// false = no insert holes in former, no hub preview in partnum 0
...
// Inner ring annulus (solid) — winder insert holes stay in this material, not in the center bore
...
// Blind holes in inner ring for brass heat-set inserts (M3): +Z face, then mirror([0,0,1]) for −Z — both faces of EACH coil.
...
// Separate print: disk + 8 mm center hole + former M3 holes + metal flange bolt holes (no plastic shaft).
module winderHubSeparatePart()
{
	difference()
	{
		cylinder( h=winderHubDiskThickness, r=winderHubDiskRadius, center=true );
		// Center: coupling / winder drive (metal flange), not printed shaft
...
module helmholtzStats()
{
	...
	if ( useCoilWinderHub )
	{
		...
		echo( str( "        Insert blind hole: d=", winderHeatInsertHoleDiameter, " mm depth ", winderHeatInsertDepth, " mm (both +Z and -Z faces); bolt circle R=", winderMountBoltCircleRadius, " mm" ) );
