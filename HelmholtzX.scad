...
// HelmholtzX.scad — X axis Helmholtz pair. Target: calibrator.ini [coils] resistance_x_ohms = 10.6 Ω (both coils in series).
// AWG 23 enameled copper; coilNumberOfTurns chosen so combined R matches ~10.6 Ω at coilRadius 105 mm (see echo output).
// Y/Z: HelmholtzY.scad, HelmholtzZ.scad
...
coilNumberOfTurns				= 37;		// Per coil; pair ≈ 10.6 Ω combined @ 20°C copper table
...
coilTargetResistanceIni			= 10.6;		// calibrator.ini [coils] resistance_x_ohms (both coils series)
...
module helmholtzStats()
{
	...
	echo( str( "    Axis: X   Target R (ini): ", coilTargetResistanceIni, " Ohms (pair, series)") );
